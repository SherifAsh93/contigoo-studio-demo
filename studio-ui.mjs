...
export function createStudioUI(ctx) {
  ...
  const mobileNotice = () => '<div class="mobile-capability-note"><strong>Mobile app · browser prototype</strong><p>Design data, pages and rules, then try the phone-sized preview. Android/iOS packages, app-store publishing and PWA installation are not connected. The initial EGP 0 price is an editable placeholder, not an approved rate.</p></div>';
  ...
  function showRuntime() {
    ...
    const mobile = runtime.appId === 'mobile';
    openDialog(`<div class="dialog-heading"><div><span class="eyebrow">${mobile ? 'PHONE-SIZED APP PREVIEW · BROWSER ONLY' : 'WORKING APP PREVIEW · LOCAL DEMO'}</span><h2>${esc(client.draft.name)} / ${esc(def.name)}</h2></div><button class="icon-button" data-action="close-dialog" aria-label="Close app preview">${icon('close')}</button></div>${mobile ? '<p class="mobile-preview-note">Mobile layout preview. Native packages and PWA installation are not connected.</p>' : ''}<div class="app-runtime"><nav class="runtime-nav" aria-label="Generated app pages">${def.pages.map(item => `<button class="${item.id === page.id && !runtime.form ? 'active' : ''}" data-runtime-page="${item.id}">${esc(item.name)}</button>`).join('')}</nav><main class="runtime-body">${body}${logs.length ? `<aside class="runtime-log"><h3>Workflow activity</h3>${logs.map(log => `<p>${icon('check', 13)} ${esc(log.note)}</p>`).join('')}</aside>` : ''}</main></div><p class="tiny-note muted">This is the generated app—not a screenshot. Try its forms and simple rules. Data, quotes and publication remain browser-local; production security and deployment are not connected.</p>`, mobile ? 'runtime-dialog mobile-runtime-dialog' : 'runtime-dialog');
  ...
  return { editorView, pricingView, quoteBanner, appPrice, bind, handle, money, openEditor, customAppDialog, mobileNotice };
}
