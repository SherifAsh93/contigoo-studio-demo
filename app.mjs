...
function clientDialog(id = '', onSaved = null, onCancel = null) {
  ...
  openDialog(`<div class="dialog-heading client-dialog-header"><div><div class="eyebrow">CLIENT DIRECTORY</div><h2 id="client-dialog-title">${id ? 'Edit client details' : 'Create client'}</h2></div><button class="icon-button" data-action="close-dialog" aria-label="Close dialog">${icon('close')}</button></div>
    <form id="client-details-form" aria-labelledby="client-dialog-title">
      <div class="client-dialog-body"><p class="muted">Save the client’s details now. You can add projects whenever you are ready. Use synthetic details in this browser-local demo.</p>
        <div class="client-form-grid">${[['name', 'Client / business name', 60, 'text'], ['industry', 'Industry', 60, 'text'], ['contact', 'Contact person', 80, 'text'], ['email', 'Contact email', 120, 'email'], ['phone', 'Phone', 40, 'tel'], ['address', 'Address', 240, 'text']].map(([key, label, limit, type]) => `<div class="client-form-field ${['name', 'address'].includes(key) ? 'full-width' : ''}"><label class="field-label" for="client-${key}">${label}${key === 'name' ? ' *' : ''}</label><input class="input" id="client-${key}" name="${key}" type="${type}" maxlength="${limit}" value="${esc(client[key] || '')}" ${key === 'name' ? 'required autofocus' : ''}></div>`).join('')}
          <div class="client-form-field full-width"><label class="field-label" for="client-notes">Notes</label><textarea class="input" id="client-notes" name="notes" rows="3" maxlength="2000">${esc(client.notes || '')}</textarea></div>
        </div>
      </div>
      <div class="client-dialog-footer"><p class="form-error" role="alert"></p><div class="dialog-actions"><button class="button secondary" type="button" id="cancel-client">${onCancel ? 'Back to project setup' : 'Cancel'}</button><button class="button primary" type="submit">${id ? 'Save client details' : 'Create client'}</button></div></div>
    </form>`, 'client-details-dialog');
